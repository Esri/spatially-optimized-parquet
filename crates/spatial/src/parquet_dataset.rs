...
mod tests {
  ...
  #[test]
  fn recursively_discovers_partitioned_output() {
    let temp = TempDir::new().unwrap();
    let partition = temp.path().join("z_order=42");
    fs::create_dir_all(&partition).unwrap();
    fs::write(partition.join("part.parquet"), []).unwrap();

    let files = discover_parquet_dataset(temp.path(), DiscoveryMode::Recursive)
      .unwrap()
      .unwrap();
  ...
  #[test]
  fn flat_discovery_ignores_nested_files() {
    let temp = TempDir::new().unwrap();
    fs::create_dir_all(temp.path().join("nested")).unwrap();
    fs::write(temp.path().join("root.parquet"), []).unwrap();
    fs::write(temp.path().join("nested/child.parquet"), []).unwrap();

    let files = discover_parquet_dataset(temp.path(), DiscoveryMode::Flat)
      .unwrap()
      .unwrap();

    assert_eq!(files.len(), 1);
    assert_eq!(files[0].relative_path, PathBuf::from("root.parquet"));
  }
}
