...
impl ParquetDataset {
  ...
  pub(crate) fn discover(input: &Path, mode: DiscoveryMode) -> Result<Option<Self>, InputError> {
    if input.is_file() {
      ...
      let relative_path = path.file_name().map(PathBuf::from).ok_or_else(|| {
        InputError::Metadata(format!(
          "parquet file path is missing a file name: {}",
          path.display()
        ))
      })?;
      return Ok(Some(Self {
    ...
    let files = paths
      ...
      .map(|path| {
        let relative_path = path
          ...
          .map_err(|error| {
            InputError::Metadata(format!(
              "resolve relative parquet path {} from {}: {error}",
              path.display(),
              root.display()
            ))
          })?
  ...
  fn discover_directory_files(
    ...
    let entries = fs::read_dir(directory).map_err(|source| InputError::Path {
      operation: "read directory",
      path: directory.to_path_buf(),
      source,
    })?;
    for entry in entries {
      let entry = entry.map_err(|source| InputError::Path {
        operation: "read directory entry",
        path: directory.to_path_buf(),
        source,
      })?;
      let file_type = entry.file_type().map_err(|source| InputError::Path {
  ...
  fn to_absolute_path(path: &Path) -> Result<PathBuf, InputError> {
    if path.is_absolute() {
      Ok(path.to_path_buf())
    } else {
      Ok(
        std::env::current_dir()
          .map_err(|source| InputError::Path {
...
impl PartitionDescriptor {
  pub(crate) fn parse(relative_path: &Path) -> Result<Option<Self>, InputError> {
  ...
  fn parse_component(component: &str) -> Result<Option<Self>, InputError> {
    for family in [PartitionFamily::Z, PartitionFamily::Xz] {
      ...
      if let Some(value) = component.strip_prefix(&prefix) {
        let lower_bound = value
          .parse::<u64>()
          .map_err(|source| InputError::ParseInteger {
...
impl ParquetDatasetFile {
  ...
  pub(crate) fn load_metadata(&self) -> Result<ArrowReaderMetadata, InputError> {
    ArrowReaderMetadata::load(
      &fs::File::open(&self.path).map_err(|source| InputError::Path {
