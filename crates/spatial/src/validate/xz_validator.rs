...
use crate::geometry::{
  Extent2D, GeometryFamily as GeometryType, NativeGeometryArrayBuilder, PbfGeometry, WkbCoordinate,
};
...
impl XzValidator {
  ...
  pub(crate) fn validate_file(
    ...
    let read_result = file.read_row_groups(&projected_columns, |row_group, row_offset, batch| {
      ...
      for row_index in 0..batch.num_rows() {
                ...
                match GeometryValidator::inspect(&payload) {
                  ...
                  Err(error) => {
                    report.push(
                      ...
                      ValidationSeverity::Error,
                      level_location,
                      format!("invalid multiscale WKB geometry: {error}"),
                    );
                    continue;
                  }
                }
              }
              let decoded = match PbfGeometry::from_bytes(&payload) {
                Ok(decoded) => decoded,
                Err(error) => {
                  report.push(
                    ValidationRule::Pbf,
        ...
        if native_geometry {
          let feature_extent = match CoveringExtentView::resolve(&covering_view, row_index) {
        ...
        for (level_index, level) in index.levels.iter().enumerate() {
          ...
          let level_location = ValidationLocation::file(file.file.relative_path.clone())
            ...
            .with_row(row)
            .with_column(level_paths[level_index].clone());
          Self::validate_pbf_vertex_provenance(
            &bytes,
  ...
  fn validate_pbf_vertex_provenance(
    ...
    if geometry_type
      .visit_wkb_for_display(wkb, &mut collector)
      .is_err()
    {
      return;
    }
  ...
  fn quantized_component(value: Option<f64>, scale: f64, translate: f64) -> i64 {
    value
      ...
      .and_then(|value| Self::quantized_value(value, scale, translate))
      .unwrap_or(0)
  }
...
impl<'array> CoveringExtentView<'array> {
  fn from_batch(
  ...
  fn resolve(
    view: &Result<Option<Self>, String>,
    row_index: usize,
    ...
    view.extent_at(row_index)
  }

  fn extent_at(&self, row_index: usize) -> Result<Option<Extent2D>, String> {
    let values = [&self.xmin, &self.ymin, &self.xmax, &self.ymax];
...
impl XzValidator {
  ...
  fn warn_pbf_winding(
    ...
    for (part_index, length) in lengths.iter().copied().enumerate() {
      ...
      let area = Self::signed_area(&absolute);
      if area == 0 {
        continue;
      }
