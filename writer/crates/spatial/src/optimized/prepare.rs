...
use crate::geoparquet::resolve_source_context;
...
pub(crate) struct SpatialPlanningContext {
  ...
  pub(crate) reprojection_context: ReprojectionContext,
...
pub(crate) struct PreparedOptimizeOutput {
  ...
  pub(crate) retained_cluster_key_column: Option<&'static str>,
}
...
pub(crate) async fn build_spatial_planning_context(
  ...
  let reprojection_context =
    ReprojectionContext::from_source_projjson(source_projjson, request.output_wkid)?;
...
pub(crate) async fn prepare_optimized_output(
  ...
  let partition_column =
    (request.output_layout.parts > 1).then_some(cluster_partition_column(&analysis));
  validate_cluster_partition_column(request.source_schema, partition_column)?;
  let ordered_request = OrderedDataframeRequest {
    ...
    transform: planning.reprojection_context.transform(),
  };
  ...
  let retained_cluster_key_column =
    if partition_column.is_some() && matches!(analysis.geometry_family, GeometryFamily::NonPoint) {
      Some(cluster_key_column(&analysis))
    } else {
      None
    };
...
async fn analyze_optimized_geometry(
  ...
  let analysis = if let Some(analysis) = metadata_display_analysis(
    ...
    planning
      ...
      .clone(),
    request.row_range.is_full() && !planning.reprojection_context.requires_reprojection(),
  ) {
    explain_stage_note(
      request.explain,
      "Analyzing geometry",
    ...
    let helper_dataframe = build_narrow_helper_projection_dataframe(
      ...
      &planning.geometry_spec,
      planning.geometry_type,
      planning.reprojection_context.transform(),
    )?;
