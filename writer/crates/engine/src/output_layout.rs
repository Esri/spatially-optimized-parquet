...
//! directory and therefore requires an explicit part count. [`resolve_output_layout`] enforces
...
pub fn resolve_output_layout(
  ...
  let parts = if is_directory {
    let parts = output_files.ok_or(OutputLayoutError::OutputFilesRequired)?;
    if parts == 0 {
      return Err(OutputLayoutError::OutputFilesInvalid.into());
    }
    parts
  } else {
    let parts = output_files.unwrap_or(1);
    if parts != 1 {
      return Err(OutputLayoutError::OutputFilesMustBeOne.into());
    }
    parts
  };
  ...
  if output.exists() {
    if !overwrite {
      return Err(OutputLayoutError::OutputExists(output.to_path_buf()).into());
    }
...
/// Resolve a validated layout into deterministic output file paths.
pub fn resolved_output_paths(layout: &OutputLayout) -> Result<Vec<PathBuf>> {
  if layout.is_directory {
    let mut paths = Vec::new();
    for idx in 0..layout.parts {
      let filename = format!("part-{idx:05}.parquet");
      paths.push(layout.path.join(filename));
    }
    Ok(paths)
  } else {
    Ok(vec![layout.path.clone()])
  }
}
