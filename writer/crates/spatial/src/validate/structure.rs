...
pub(crate) fn validate_dataset_structure(
  ...
  for (file, contract) in files.iter().zip(contracts) {
    ...
    match &contract.geodisplay {
      GeodisplayMetadata::Z(index) => super::z::validate_z_schema(file, index, report),
      GeodisplayMetadata::Xz(index) => super::xz::validate_xz_schema(file, index, report),
    }
...
pub(crate) fn validate_file_data(
  ...
  for (file, contract) in files.iter().zip(contracts) {
    ...
    let range = match &contract.geodisplay {
      GeodisplayMetadata::Z(index) => super::z::validate_z_file(file, contract, index, report),
      GeodisplayMetadata::Xz(index) => super::xz::validate_xz_file(file, contract, index, report),
    };
...
fn validate_partition_family(
  ...
  let expected = match &contract.geodisplay {
    GeodisplayMetadata::Z(_) => PartitionFamily::Z,
    GeodisplayMetadata::Xz(_) => PartitionFamily::Xz,
  };
...
fn validate_clustering_page_indexes(
  ...
  let column_path = match &contract.geodisplay {
    GeodisplayMetadata::Z(index) => index.code.clone(),
    GeodisplayMetadata::Xz(index) => format!("{}.{}", index.field, index.code),
  };
